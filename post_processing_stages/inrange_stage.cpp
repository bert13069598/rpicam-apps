...
private:
	...
	int low_hue1, low_hue2, high_hue1, high_hue2, range_count;
	Scalar color;
};
...
bool InrangeStage::Process(CompletedRequestPtr &completed_request)
{
	...
	inRange(imageMat, Scalar(low_hue1, 50, 50), Scalar(high_hue1, 255, 255), img_mask1);
	if (range_count == 2) {
		inRange(imageMat, Scalar(low_hue2, 50, 50), Scalar(high_hue2, 255, 255), img_mask2);
		img_mask1 |= img_mask2;
	}
	...
	resize(img_mask1, img_mask1, Size(info.width / 4, info.height / 4), 0, 0, INTER_LINEAR);
	imshow("Mask Image", img_mask1);
	waitKey(10);
